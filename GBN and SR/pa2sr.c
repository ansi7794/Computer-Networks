...
// Variables for reciever or sender or for statistical purpose
...
int cannotSend = 0;  // Prevents A_output from sending spam print statements about not being able to send due to window size limits
int sentToLayer5 = 0;
int totalMsgsSent = 0;
...
int checkSum (packet)  // function for checksum
  struct pkt packet;
{
    int checksum = 0;
    int i;
    ...
    struct msg message;
{
    if (seqNum < numACKS + WINDOW_SIZE) {
        msgBuffer[seqNum].seqnum = seqNum; // Sequence number is set
        msgBuffer[seqNum].acknum = seqNum; // Acknowledgement number is set
        int i;
        for (i = 0; i < 20; i++) {
            msgBuffer[seqNum].payload[i] = message.data[i];  // Message placed in payload of the packet
        }
        msgBuffer[seqNum].checksum = checkSum(msgBuffer[seqNum]);
        totalMsgsSent += 1;
        tolayer3(A, msgBuffer[seqNum]); // Packet with message to B is sent through layer 3
        printf("A_output - message %d sent\n", msgBuffer[seqNum].seqnum); 
        //Timer starts only for first packet of a window
        if (seqNum == numACKS) { 
  ...
  struct pkt packet;
{
    ...
    if (packet.checksum == checksum) { // Checksum of received ACK packet for detecting corruption
        int oldNum = numACKS;
        if (packet.acknum >= numACKS + 1) {
            printf("A_input - ACK %d received\n", packet.acknum);
            numACKS += packet.acknum - numACKS;     // Uncorrupted ACKS received
    ...
    struct pkt packet;
{
    ...
    if (packet.checksum == checksum) {
        ...
        if (packet.seqnum == expectedSeqNum) {
            ...
            if (bState == 0 || bState == packet.seqnum) {
...
void Simulation_done()
{
    ...
    float i = numLostMsgs*100/totalMsgsSent;
    printf("Ratio of lost packets: %6.2f%%\n", i);
    i = numCorruptMsgs*100/totalMsgsReceived;
    printf("Ratio of corrupted packets: %6.2f%%\n", i);
    double totalRTT = 0;
    int numRTT = 0;
    double avgRTT;
    int j;
    ...
    printf("Average communication time: %.3f\n", communicate);
    printf("==================================================");
