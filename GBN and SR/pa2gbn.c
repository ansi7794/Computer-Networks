...
// For A
int txBase; // first sequence number
int nextSeqNum; // last sequence number
int nextMsg; // next message
msg *msgBuffer;
pkt *txPktBuffer;
int msgCount;
...
{
    printf("A: Recieve message from above...\n");
    printf(" DATA: %.*s\n", 19, message.data);
    printWindow(txBase);
...
void A_input(pkt packet)
{
    ...
    {
        // Discard packet
        printf("  A rejected ACK from B... (pending ACK %d)\n", txBase);
        printWindow(txBase);
    }
}
...
void A_init(void)
{
    ...
    txPktBuffer = (pkt *) malloc(sizeof(pkt) * LIMIT_SEQNO);
    ...
    corruptPktCount = 0;
    rxmtCount = 0;
    txPktTime = (double *) malloc(sizeof(double) * LIMIT_SEQNO);
    totalTtc = 0;
    commPktCount = 0;
    receivedMsgCount = 0;
...
void Simulation_done()
{
    ...
    printf("Ratio of lost packets:  %f", lossRate);
    printf("Ratio of corrupted packets: %f", corruptRate);
